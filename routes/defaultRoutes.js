...
router.route("/topic").get(defaultController.getTopicPosts);
      ...
      User.findOne({ email: email }).then((user) => {
        if (!user) {
          return done(
            ...
            req.flash(
              "error-message",
              "User not found with this email, please try signing in..."
            )
          );
        }
...
router.get("/question", defaultController.getQuestion);
router.get("/answer", defaultController.getAnswer);
